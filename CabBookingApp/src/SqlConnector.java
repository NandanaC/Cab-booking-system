...
public class SqlConnector {
	...
	public static String getEmailid(String id)
	{
		...
		try{  
				...
				"jdbc:mysql://localhost:3306/cabbooking","root","fountainheadd");   
				Statement stmt=con.createStatement(); 
				String que = "select emailid from User where userid = '"+ id + "';";
				rs = stmt.executeQuery(que);
				while(rs.next())
	...
	public static void updateAvailabilityN(String id) {
		String query = "update driver set availability = 'No' where vehicleno = '"+ id + "';";
		try{  
			Class.forName("com.mysql.cj.jdbc.Driver");  
			Connection con=DriverManager.getConnection(  
			"jdbc:mysql://localhost:3306/cabbooking","root","fountainheadd");    
	...
	public static void updateBalance(String id,double wallet) {
		...
		public static String getAvailability(String id)
		{
			...
			try{  
					...
					"jdbc:mysql://localhost:3306/cabbooking","root","fountainheadd");   
					Statement stmt=con.createStatement(); 
					String que = "select availability from driver where vehicleno = '"+ id + "';";
					rs = stmt.executeQuery(que);
					while(rs.next())
